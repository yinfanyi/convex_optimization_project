...
Parser = inputParser;
addParameter(Parser, 'filename', 'lena_512.png');
...
% if ismember('noise_image_RGB', fieldnames(Parser.Results))
%     u = double(noise_image_RGB);
% else
%     u = double(imread(filename));
% end
if isfield(Parser.Results, 'noise_image_RGB')
    % 用户输入了特定的函数参数"noise_image_RGB"
